...
export function normalizeNewReading(body, { allowLegacy = false } = {}) {
  ...
  if (rules.ranges) {
    ...
    if (invalidPosition !== -1) {
      ...
      const [positionCn, positionEn] = rules.positions[invalidPosition];
      return {
        error: `第 ${invalidPosition + 1} 位「${positionCn}」只接受 ${min}–${max} 号牌。 · Position ${invalidPosition + 1} (${positionEn}) only accepts cards ${min}–${max}.`,
      };
    }
  }
