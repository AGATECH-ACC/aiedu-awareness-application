...
/** @param {SupabaseClient} supabase */
export async function getEducatorQualifyingReportCount(supabase) {
  const { data, error } = await supabase.rpc('educator_qualifying_report_count');
  rethrow(error, 'get educator qualifying report count');
  const count = Number(data);
  if (!Number.isSafeInteger(count) || count < 0) {
    throw new Error('Invalid educator qualifying report count');
  }
  return count;
}
