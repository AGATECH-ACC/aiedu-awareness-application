...
export function sendRecipientOtpEmail({ name, email, code, expiresMinutes = 10 }) {
  ...
  return sendResendEmail({
    ...
    html: emailFrame({
      ...
      body: `请把以下六位代码告诉正在为你进行觉察卡抽牌的教育者。<br><span style="color:#766149">Share this six-digit code with the educator conducting your reading.</span>${brandedEmailCode(safeCode)}代码将在 ${expiresMinutes} 分钟后失效，并且只能用于一份报告。<br><span style="color:#766149">It expires in ${expiresMinutes} minutes and authorizes one report only.</span>`,
    }),
  });
}
...
export function sendRecipientReportEmail({ name, email, reportUrl, reading = null }) {
  ...
  const spreadSummary = reading?.mode
    ? `${escapeHtml(readingSpreadLabel(reading.mode, reading.spread_key))}${cardCount ? ` · ${cardCount} 张牌 / ${cardCount} cards` : ''}`
    : '';
  const action = brandedEmailAction({
    href: safeUrl,
    label: '查看我的报告 · View my report',
  });
  return sendResendEmail({
    to: email,
    ...
    html: emailFrame({
      ...
      body: `感谢你确认这次抽牌。你可以通过下方的私人链接阅读完整的双语觉察报告。<br><span style="color:#766149">Thank you for confirming this reading. Use the private link below to read your complete bilingual awareness report.</span>${spreadSummary ? `<div style="margin-top:18px;padding:12px 14px;border:1px solid #dfcfac;border-radius:10px;background:#fffaf0;color:#56452e;font-weight:700">${spreadSummary}</div>` : ''}`,
      action,
    }),
  });
}
