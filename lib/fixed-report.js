import { byNum, CHAPTERS, INNER_CHILD, SPREAD3 } from './cards.js';
import { insightFor } from './card-insights.js';

export const FIXED_REPORT_VERSION = 'fixed-card-insights-v1';
...
function positionMeaning(mode, index, position) {
  if (mode === 1) {
    return [
...
function resolvedCards(mode, cardNumbers, positions) {
  return cardNumbers.map((number, index) => {
    ...
    return {
      ...
      positionMeaning: positionMeaning(mode, index, positions[index]),
    };
  });
}
...
/**
 * Build the Version 1 report without calling an external model.
...
export function buildFixedReport({ mode, spreadKey, positions, cardNumbers, question = '' }) {
  const items = resolvedCards(mode, cardNumbers, positions);
...
整组牌最重要的连接，是从「看见」走向「选择」：先辨认当下模式，再理解它试图保护或提醒什么，最后实践一个较健康、较真实的回应。

The spread's most important connection is the movement from noticing to choosing: recognise the present pattern, understand what it is protecting or highlighting, then practise one healthier and more authentic response.
...
1. **看见 · Notice:** 不急着评判，先辨认哪一张牌最像你此刻的状态。 / Without judging, identify which card most closely resembles your present state.
