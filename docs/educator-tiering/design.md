...
For both `为自己抽牌` and `为他人抽牌`, basic educators can use only the single- and two-card modes. At 30 qualifying reports, advanced educators can use all current modes. The UI limits the available controls, and `POST /api/report` repeats the check before it writes a reading, report, delivery, or consumes a recipient authorization. Non-educator and public report behavior are unchanged.
